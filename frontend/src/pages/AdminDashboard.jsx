        ...
        {tab === 'overview' && stats && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
              ...
              <StatCard label="High Risk Scans" value={stats.highRiskScans} sub={`${stats.totalScans ? Math.round(stats.highRiskScans / stats.totalScans * 100) : 0}% of total`} color='#A32D2D' />
              <StatCard label="Active Users"    value={stats.activeUsers} sub="scanned at least once" />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
              <StatCard label="Total Users"      value={stats.totalUsers} sub="registered" />
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: '14px 16px', border: '0.5px solid #e0e0d8' }}>
              ...
              {Object.entries(stats.riskBreakdown || {}).map(([level, count]) => (
                <div key={level} style={{ display: 'grid', gridTemplateColumns: '80px 1fr 40px', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                  ...
                  <Bar value={count} max={stats.totalScans} color={RISK_COLOR[level]} />
                  ...
                  <tr key={i} style={{ borderBottom: '0.5px solid #f8f8f4' }}>
                    ...
                    <td style={{ padding: '8px 10px' }}>
                      <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 10,
                                      background: RISK_BG[s.risk] || '#f0f0e8', color: RISK_COLOR[s.risk] || '#555' }}>
                        {s.risk}
                      </span>
                    </td>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
              ...
              <StatCard label="Last Scrape"
                value={scraper?.lastScrapeAt ? new Date(scraper.lastScrapeAt).toLocaleDateString('en-IN') : '—'}
                sub={scraper?.lastScrapeAt ? new Date(scraper.lastScrapeAt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) : 'never'} />
            </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button onClick={triggerScraper} disabled={triggering}
                  style={{ padding: '8px 18px', borderRadius: 8, border: 'none', cursor: triggering ? 'not-allowed' : 'pointer',
                           background: triggering ? '#ccc' : '#1a3d2b', color: '#fff', fontSize: 12, fontWeight: 500 }}>
